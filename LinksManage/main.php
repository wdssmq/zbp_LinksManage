...
if ('save' == GetVars('act', 'GET')) {
    ...
    $mod->Save();
    $zbp->SetHint('good');
    Redirect('1' == $_POST['stay'] ? $_SERVER['HTTP_REFERER'] : '../../../zb_system/cmd.php?act=ModuleMng');
}
...
if ($edit = GetVars('edit', 'GET')) {
    ...
    if (strlen($file_contents) > 0 && $items = json_decode($file_contents)) {
        ...
        if ($tree) {
            $content = str_replace(['<ul>', '</ul></li>'], ["</li>\n", ''], $content);
        }
        $preg = [
            'tag'    => '/<li.*?<\/li>/',
            'sub'    => '/<li.*?class=[\'|\"](.*?)[\'|\"]/i',
    ...
    if ('system' == $mod->Source || 'theme' == $mod->Source || '' !== $mod->FileName) {
        $islock = 'readonly="readonly"';
    }
    $delbtn = 'plugin_LinksManage' === $mod->Source ? '&nbsp;<a title="删除当前模块"
